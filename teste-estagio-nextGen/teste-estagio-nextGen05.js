...
function calculaPorcentagemSorteio(assinante,minutosAssistidos) {
    ...
    for (let i=0;i<assinante.length;i++){
        ...
        if (assinante[i])
            horasAssistidas[i]*=2
        somaVetor+=horasAssistidas[i]
    }
    for (let i=0;i<horasAssistidas.length;i++)
        percentual[i]=Math.round((horasAssistidas[i]/somaVetor)*100)
    console.log(minutosAssistidos,horasAssistidas,somaVetor,percentual)
    return percentual
}

calculaPorcentagemSorteio([true, false],[60, 120])
console.log(Math.ceil(181/60))